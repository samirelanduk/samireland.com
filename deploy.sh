...
docker compose -f docker-compose.yml -p sidc build next --progress=plain --no-cache && \
    ...
    ssh $HOSTNAME "cd ~/$HOSTNAME && BACKEND_URL=$BACKEND_URL FRONTEND_URL=$FRONTEND_URL MEDIA_URL=$MEDIA_URL BUILD_API_URL=$BUILD_API_URL NEXT_PUBLIC_API_URL=$NEXT_PUBLIC_API_URL TAG=$TAG HOSTNAME=$HOSTNAME NEXT_PUBLIC_ALLOW_FALLBACK=$NEXT_PUBLIC_ALLOW_FALLBACK docker compose -p sidc pull" && \
    ssh $HOSTNAME "cd ~/$HOSTNAME && BACKEND_URL=$BACKEND_URL FRONTEND_URL=$FRONTEND_URL MEDIA_URL=$MEDIA_URL BUILD_API_URL=$BUILD_API_URL NEXT_PUBLIC_API_URL=$NEXT_PUBLIC_API_URL TAG=$TAG HOSTNAME=$HOSTNAME NEXT_PUBLIC_ALLOW_FALLBACK=$NEXT_PUBLIC_ALLOW_FALLBACK docker compose -p sidc up -d" && \
