    ...
    <div className="z-10 relative">
      ...
      <div className="flex gap-3 flex-col sm:flex-row sm:gap-6">
        ...
        <div className="">
          ...
          <time className="text-slate-500 block mb-2 md:text-lg">{formatMonth(event.start)} - {formatMonth(event.end)}</time>
          <div
            dangerouslySetInnerHTML={{ __html: event.description }}
            className={`prose prose-p:my-2 max-w-none prose-p:max-w-none text-slate-800 ${isSub ? "text-sm" : "text-base"}`}
          />
        </div>
      </div>
